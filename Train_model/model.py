...
class LinearRNN(nn.Module):
    ...
    def initial_hidden(self):
        if self.algorithm == 'lstm':
            return (Variable(torch.zeros( 1,1, self.embedding_dim)).cuda(),
                    Variable(torch.zeros( 1,1, self.embedding_dim)).cuda())
        elif self.algorithm == 'gru' or self.algorithm == 'rnn':
            return Variable(torch.zeros( 1, 1,self.embedding_dim)).cuda()
        else:
            raise (NameError, "Unknown Type:" + self.algorithm)
...
class Sim_CNN_RNN(nn.Module):
    ...
    def hidden_setup(self,sentA,sentB):
        ...
        return self.sigmoid(ht)
    def initial_hidden(self):
        self.hiddenGRU = Variable(torch.zeros(self.num_layers,1,self.hidden_dim)).cuda()
        self.hiddenLSTM = (Variable(torch.zeros(1,1,self.max_pool_out_dim)).cuda(),
                           Variable(torch.zeros(1,1,self.max_pool_out_dim)).cuda())
    def initial_weights(self):
        initrange = 0.1
        self.bi_linear.weightA.data.uniform_(-initrange,initrange)
        self.bi_linear.weightB.data.uniform_(-initrange,initrange)
...
class Com_CNN_RNN(nn.Module):
    ...
    def initial_hidden(self):
        # initial hidden layer
        if self.algorithm_first == 'lstm':
            self.hidden_first = (Variable(torch.zeros(self.num_layers,1,self.embedding_dim)).cuda(),
                                 Variable(torch.zeros(self.num_layers,1,self.embedding_dim)).cuda())
        elif self.algorithm_first == 'gru' or self.algorithm_first == 'rnn':
            self.hidden_first = Variable(torch.zeros(self.num_layers,1,self.embedding_dim)).cuda()
        else:
            raise (NameError,"Unknown Type:" + self.algorithm_first)
        if self.algorithm_second == 'lstm':
            self.hidden_second = (Variable(torch.zeros(1,1,self.hidden_dim)).cuda(),
                                  Variable(torch.zeros(1,1,self.hidden_dim)).cuda())
